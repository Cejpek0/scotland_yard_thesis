...
class ScotlandYardEnvironment(MultiAgentEnv):
    ...
    def __init__(self, config):
        ...
        self._agent_ids = ["mr_x"]
        for cop in self.game.get_cops():
            self._agent_ids.append("cop_" + str(cop.number))

        # Define your environment state and other components here
        self.action_spaces = {"mr_x": spaces.Discrete(4)}
        for cop in self.game.get_cops():
            self.action_spaces["cop_" + str(cop.number)] = spaces.Discrete(4)
        ...
        self.observation_spaces = {"mr_x": spaces.Box(low=-100, high=100, shape=(mr_x_number_of_observations,))}
        for cop in self.game.get_cops():
            self.observation_spaces["cop_" + str(cop.number)] = spaces.Box(low=-100,
                                                                           high=100,
