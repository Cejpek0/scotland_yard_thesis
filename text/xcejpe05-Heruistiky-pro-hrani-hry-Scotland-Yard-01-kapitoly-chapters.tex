...
Parametry strategie se mohou upravovat například pomocí metody \textbf{gradientního  vzestupu}:
\begin{equation}
...
Tím, že se hledá lokální maximum pouze v~oblasti důvěry, problém se zaseknutím v~těchto lokálních optimech je značně redukován.
...
\begin{equation}
  \begin{split}
    J^{TRPO}(\theta) &= \sum_{s \in \mathcal{S}} d_{\pi_{\theta_{\text{old}}}}(s) \sum_{a \in \mathcal{A}} \pi_{\theta}(a \vert s) r(\theta) \hat{A}_{\theta_{\text{old}}}(s, a)\\
    \hat{A}_{\theta_{\text{old}}}(s, a) &= Q_{\pi_{\theta_{\text{old}}}}(s, a) - V_{\pi_{\theta_{\text{old}}}}(s)\\
  \end{split}
\end{equation}

\begin{myitemize}
  \item $\hat{A}_{\theta_{\text{old}}}(s, a)$ --- odhad výhody akce $a$ ve stavu $s$. Měří jak je daná akce lepší než průměrná akce ve stavu $s$.
\end{myitemize}
...
Tyto změny tvoří PPO oproti TRPO kompatibilní s~metodou gradientního vzestupu, která je velmi snadná na výpočet.
Parametry se tedy aktualuzují dle tohoto vzorce
...
\begin{algorithm}
  ...
  \For{iterace $= 0, 1, 2, \dots$}
      ...
      \[
        \theta_{k+1} = argmax_{\theta} \frac{1}{\left|\mathcal{D}_k\right|T}\displaystyle\sum_{\tau \in \mathcal{D}_k} \sum_{t=0}^T \left[ \min \left( r(\theta) \hat{A}_t, \text{clip}(r(\theta), 1-\epsilon, 1+\epsilon) \hat{A}_t \right) \right]
      \]
    \EndFor
  \end{algorithmic}
...
\subsection{\emph{Step} metoda hraní hry}
{\color{red}TODO: pseudoalgoritmus}
%
%def step():
%  current_player = get_current_player()
%  action = get_action_for_player(current_player)
%  move_player(current_player, action)
%  update_game_state()
